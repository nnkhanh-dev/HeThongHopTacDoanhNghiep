...
{
    public class ViecLamService : IViecLam
    {
        private readonly AppDbContext _context;

        public ViecLamService(AppDbContext context)
        {
            _context = context;
        }

        public async Task<PageResult<ViecLamVM>> GetListViecLam(
            int pageIndex = 1,
            int pageSize = 10,
            string? keyword = null,
            ...
            string? sapXepTheo = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(linhVucSlug))
                query = query.Where(x => x.LinhVuc.Slug == linhVucSlug);
            ...
            var items = await query
                ...
                .Select(x => new ViecLamVM
                {
                    ...
                    LinhVuc = x.LinhVuc != null ? x.LinhVuc.Ten : null,
        ...
        public async Task<PageResult<ViecLamVM>> GetRelatedViecLam(int pageIndex, int pageSize, string viecLamSlug, string? keyword, string? linhVucSlug)
        {
            var query = _context.ViecLams.AsNoTracking().Where(x => x.Slug != viecLamSlug && x.DeletedAt == null);
        ...
        public async Task<BaseResult<ViecLamVM>> GetViecLamBySlug(string slug)
        {
            var item = await _context.ViecLams
                ...
                .Select(x => new ViecLamVM
                {
                    ...
                    DoanhNghiepLogo = x.DoanhNghiep != null ? x.DoanhNghiep.Logo : null,
                    LinhVuc = x.LinhVuc != null ? x.LinhVuc.Ten : null,
                    LinhVucSlug = x.LinhVuc != null ? x.LinhVuc.Slug : null,
                    CreatedAt = x.CreatedAt,
                    UpdatedAt = x.UpdatedAt
                })
                .FirstOrDefaultAsync();
